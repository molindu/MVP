...
const OrganizerDashboard = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      const confirmedRequests = organizerRequests.filter(
        (req) => req.status === "Confirmed"
      ).length;
      const completedRequests = organizerRequests.filter(
        (req) => req.status === "Completed"
      ).length;

      // Calculate total revenue (from completed events)
      const completedEvents = organizerRequests.filter(
        (req) => req.status === "Completed"
      );
      const totalRevenue = completedEvents.reduce((sum, req) => {
        const eventPackage = organizerPackages.find(
          (pkg) => pkg.id === req.packageId
        );
                  ...
                  {eventsOnSelectedDate.map((event) => (
                    <div key={event.id} className="bg-gray-50 p-3 rounded-md">
                      <p className="font-medium">
                        {getPackageTitle(event.packageId)}
                      </p>
            ...
            {requests.length === 0 ? (
              ...
              <div className="space-y-4">
                {requests.slice(0, 5).map((request) => (
                  ...
                  >
                    <p className="font-medium">
                      {getPackageTitle(request.packageId)}
                    </p>
                    <div className="flex justify-between items-center mt-1">
                      ...
                      <span className="text-xs text-gray-500">
                        {formatTimeElapsed(request.requestDate)}
                      </span>
                    </div>
